...
template <typename K, typename V, template <typename...> class Container>
bool HashTable<K, V, Container>::insert( K key, const V& value) {
...
template <typename K, typename V, template <typename...> class Container>
Pair<K, V>* HashTable<K, V, Container>::search(const K& key) const {
    size_t index = hash_function(key);
    for (auto& pair : table[index]) {
        if (pair.key == key) {
            return &pair;
...
template <typename K, typename V, template <typename...> class Container>
inline vector<K> HashTable<K, V, Container>::keys() const{    
    std::vector<K> result;
    for (size_t i = 0; i < capacity; ++i) {
        for (const auto& pair : table[i]) {
